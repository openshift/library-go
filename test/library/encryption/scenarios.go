...
type InvalidEncryptionRecoveryScenario struct {
	BasicScenario
...
func TestInvalidEncryptionRecovery(ctx context.Context, t testing.TB, scenario InvalidEncryptionRecoveryScenario) {
	e := NewE(t, PrintEventsOnFailure(scenario.OperatorNamespace))
	clientSet := GetClients(e)

	steps := []testStep{
		{name: "ApplyInvalidConfig", testFunc: func(t testing.TB) {
			if scenario.InvalidProvider.Setup != nil {
		...
		{name: "ApplyValidConfigAndVerifyRecovery", testFunc: func(t testing.TB) {
			SetAndWaitForEncryptionType(ctx, t, scenario.ValidProvider,
