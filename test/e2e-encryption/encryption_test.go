...
func TestEncryptionIntegration(tt *testing.T) {
	...
	t.Logf("In-place KMS plugin config update: change image on key 12 (non-migration field)")
	...
	err = wait.PollUntilContextTimeout(ctx, time.Millisecond*500, wait.ForeverTestTimeout, true, func(ctx context.Context) (bool, error) {
		s, err := kubeClient.CoreV1().Secrets("openshift-config-managed").Get(ctx, fmt.Sprintf("encryption-key-%s-%s", component, "12"), metav1.GetOptions{})
		if err != nil {
			return false, err
		}
		...
		if err != nil {
			return false, fmt.Errorf("failed to decode kms-plugin-config for key 12: %w", err)
		}
	...
	verifyKMSPlugins()
	// The in-place plugin config update on key 12 triggers the state controller to
