...
type VaultSecretReference struct {
	...
	// +kubebuilder:validation:XValidation:rule="!format.dns1123Subdomain().validate(self).hasValue()",message="name must be a valid DNS subdomain name"
	// +required
...
type VaultKMSConfig struct {
	...
	// +kubebuilder:validation:XValidation:rule="self != 'root' && self != 'sys' && self != 'audit' && self != 'auth' && self != 'cubbyhole' && self != 'identity'",message="vaultNamespace cannot be a reserved string (root, sys, audit, auth, cubbyhole, identity)"
	// +optional
	...
	//
	// +required
	ApproleSecret VaultSecretReference `json:"approleSecret,omitzero"`
...
type VaultTLSConfig struct {
	...
	//
	// The value must be a valid DNS hostname.
	//
	// +kubebuilder:validation:MaxLength=253
	// +kubebuilder:validation:MinLength=1
	// +kubebuilder:validation:XValidation:rule="!format.dns1123Subdomain().validate(self).hasValue()",message="serverName must be a valid DNS hostname"
	// +optional
