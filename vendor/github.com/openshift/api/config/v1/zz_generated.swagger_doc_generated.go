...
var map_VaultConfigMapReference = map[string]string{
	...
	"name": "name is the metadata.name of the referenced ConfigMap in the openshift-config namespace. The name must be a valid DNS subdomain name.",
}
...
var map_VaultKMSConfig = map[string]string{
	...
	"approleSecret":  "approleSecret references a secret in the openshift-config namespace containing the AppRole credentials used to authenticate with Vault. The secret must contain two keys: \"roleID\" for the AppRole Role ID and \"secretID\" for the AppRole Secret ID.\n\nThe namespace for the secret referenced by approleSecret is openshift-config.",
	"transitMount":   "transitMount specifies the mount path of the Vault Transit engine. The value must be between 1 and 1024 characters when specified.\n\nWhen omitted, this means the user has no opinion and the platform is left to choose a reasonable default. These defaults are subject to change over time.\n\nThe mount path cannot start or end with a forward slash, cannot contain spaces, and cannot contain consecutive forward slashes.",
...
var map_VaultSecretReference = map[string]string{
	...
	"name": "name is the metadata.name of the referenced secret in the openshift-config namespace. The name must be a valid DNS subdomain name.",
}
