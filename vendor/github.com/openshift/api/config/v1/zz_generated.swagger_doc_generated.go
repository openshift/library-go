...
var map_VSpherePlatformTopology = map[string]string{
	...
	"networks":       "networks is the list of port group network names within this failure domain. If feature gate MultiVSphereNetworks is enabled, up to 10 network adapters may be defined. https://configmax.esp.vmware.com/guest?vmwareproduct=vSphere&release=vSphere%208.0&categories=1-0 The available networks (port groups) can be listed using `govc ls 'network/*'` Networks should be in the form of an absolute path: /<datacenter>/network/<portgroup>.",
