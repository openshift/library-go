...
// GODEBUG=fips140=on). See the KNOWN DIVERGENCE note in fips.go for why.
func TestFIPSApprovedTLSGroups(t *testing.T) {
	// Per-group approval: a single-element input returns that group iff approved.
	perGroup := []struct {
		name         string
	...
	// Filtering a mixed list drops the non-approved groups while preserving the
