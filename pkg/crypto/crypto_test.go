...
// Used in tests to assert that x509.CreateCertificate infers the correct signature algorithm.
func signatureAlgorithmForKey(t testing.TB, key crypto.PrivateKey) x509.SignatureAlgorithm {
	t.Helper()
	switch key.(type) {
...
func newSigningCertificateTemplate(subject pkix.Name, lifetime time.Duration, currentTime func() time.Time, algorithm KeyAlgorithm) *x509.Certificate {
	if lifetime <= 0 {
	...
	return newSigningCertificateTemplateForDuration(subject, lifetime, currentTime, nil, nil, algorithm)
}
...
func buildCA(t *testing.T) (crypto.PrivateKey, *x509.Certificate) {
	...
	caTemplate := newSigningCertificateTemplate(pkix.Name{CommonName: "CA"}, certificateLifetime, time.Now, AlgorithmRSA)
...
func buildIntermediate(t *testing.T, signingKey crypto.PrivateKey, signingCrt *x509.Certificate) (crypto.PrivateKey, *x509.Certificate) {
	...
	intermediateTemplate := newSigningCertificateTemplate(pkix.Name{CommonName: "Intermediate"}, certificateLifetime, time.Now, AlgorithmRSA)
...
func buildServer(t *testing.T, signingKey crypto.PrivateKey, signingCrt *x509.Certificate) (crypto.PrivateKey, *x509.Certificate) {
	...
	serverTemplate := newServerCertificateTemplate(pkix.Name{CommonName: "Server"}, hosts, certificateLifetime, time.Now, nil, nil, AlgorithmRSA)
...
func TestRandomSerialGenerator(t *testing.T) {
	...
	template := newServerCertificateTemplate(pkix.Name{CommonName: hostnames[0]}, hostnames, certificateLifetime, time.Now, nil, nil, AlgorithmRSA)
...
func TestValidityPeriodOfServerCertificate(t *testing.T) {
	...
	expirationDate := cert.NotAfter
	expectedExpirationDate := currentTime.Add(lifetime)
	if expectedExpirationDate != expirationDate {
...
func TestValidityPeriodOfSigningCertificate(t *testing.T) {
	...
	for _, test := range tests {
		cert := newSigningCertificateTemplate(pkix.Name{CommonName: "CA"}, test.passedDuration, currentFakeTime, AlgorithmRSA)
		expirationDate := cert.NotAfter
...
func TestECDSAKeyGeneration(t *testing.T) {
	...
	require.True(t, publicKey.Equal(&privateKey.PublicKey), "public key should match private key's public key")
}
...
func TestSignatureAlgorithmForKey(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			key := tt.keyGen()
			sigAlg := signatureAlgorithmForKey(t, key)
...
func TestServerCertWithECDSA(t *testing.T) {
	...
	caTemplate := newSigningCertificateTemplate(pkix.Name{CommonName: "test-ca"}, DefaultCACertificateLifetimeDuration, time.Now, AlgorithmRSA)
	...
	// Verify ECDSA cert does not have KeyEncipherment (RFC 5480 Section 3)
	require.Equal(t, x509.KeyUsageDigitalSignature, serverCert.Certs[0].KeyUsage, "ECDSA cert should only have DigitalSignature")
...
func TestMixedCAAndServerAlgorithms(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			caTemplate := newSigningCertificateTemplate(pkix.Name{CommonName: "test-ca"}, DefaultCACertificateLifetimeDuration, time.Now, tt.caAlgorithm)
			caTemplate.SubjectKeyId = caPublicKeyHash
			caTemplate.AuthorityKeyId = caPublicKeyHash
			caCert, err := signCertificate(caTemplate, caPublicKey, caTemplate, caPrivateKey)
			require.NoError(t, err)
			...
			expectedServerSigAlg := signatureAlgorithmForKey(t, caPrivateKey)
...
func TestMakeSelfSignedCAConfigForDurationWithAlgorithm(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectedSigAlg:   x509.SHA256WithRSA,
			expectedKeyUsage: x509.KeyUsageKeyEncipherment | x509.KeyUsageDigitalSignature | x509.KeyUsageCertSign,
		},
		{
			name:             "ECDSA CA",
			algorithm:        AlgorithmECDSA,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Equal(t, tt.expectedKeyUsage, caCert.KeyUsage, "KeyUsage should match algorithm")
