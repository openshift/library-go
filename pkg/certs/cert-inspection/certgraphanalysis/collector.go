		...
		for _, configMap := range configMapList.Items {
			...
			if details != nil {
				...
				if metadata, err := meta.Accessor(&configMap); err != nil {
					errs = append(errs, fmt.Errorf("failure getting metadata for ns/%v configmap/%v: %w", configMap.Namespace, configMap.Name, err)
				} else {
		...
		for _, secret := range secretList.Items {
			...
			if details != nil {
				...
				if metadata, err := meta.Accessor(&secret); err != nil {
					errs = append(errs, fmt.Errorf("failure getting metadata for ns/%v secret/%v: %w", secret.Namespace, secret.Name, err)
				} else {
					if revisionNamespace, revisionName, revisioned := isRevisioned(metadata); revisioned {
