...
func (c RotatedSigningCASecret) EnsureSigningCertKeyPair(ctx context.Context) (*crypto.CA, bool, error) {
	...
	if creationRequired {
		...
		if klog.V(2).Enabled() {
			...
			if diff := cmp.Diff(nil, actualSigningCertKeyPairSecret.Annotations, cmpopts.EquateEmpty()); len(diff) > 0 {
		...
		if klog.V(2).Enabled() {
			...
			if diff := cmp.Diff(originalSigningCertKeyPairSecret.Annotations, actualSigningCertKeyPairSecret.Annotations, cmpopts.EquateEmpty()); len(diff) > 0 {
				klog.V(2).Infof("Secret %s/%s annotations diff: %s", actualSigningCertKeyPairSecret.Namespace, actualSigningCertKeyPairSecret.Name, diff)
			}
		}
