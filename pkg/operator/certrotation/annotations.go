...
const (
	...
	// CertificateAutoRegenerateAfterOfflineExpiryAnnotation contains a link to PR and an e2e test name which verifies
	...
	// CertificateRefreshPeriodAnnotation is an e2e test name which verifies that TLS artifact is created and used correctly
...
type AdditionalAnnotations struct {
	...
	TestName string
	// AutoRegenerateAfterOfflineExpiry contains a link to PR
...
func (a AdditionalAnnotations) EnsureTLSMetadataUpdate(meta *metav1.ObjectMeta) bool {
	...
	if len(a.AutoRegenerateAfterOfflineExpiry) > 0 && meta.Annotations[CertificateAutoRegenerateAfterOfflineExpiryAnnotation] != a.AutoRegenerateAfterOfflineExpiry {
		...
		klog.V(2).Infof("Updating %q annotation for %s/%s, diff: %s", CertificateAutoRegenerateAfterOfflineExpiryAnnotation, meta.Name, meta.Namespace, diff)
