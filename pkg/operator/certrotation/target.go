...
func (c RotatedSelfSignedCertKeySecret) EnsureTargetCertKeyPair(ctx context.Context, signingCertKeyPair *crypto.CA, caBundleCerts []*x509.Certificate) (*corev1.Secret, error) {
	...
	if creationRequired {
		...
		if klog.V(2).Enabled() {
			...
			if diff := cmp.Diff(nil, actualTargetCertKeyPairSecret.Annotations, cmpopts.EquateEmpty()); len(diff) > 0 {
		...
		if klog.V(2).Enabled() {
			...
			if diff := cmp.Diff(originalTargetCertKeyPairSecret.Annotations, actualTargetCertKeyPairSecret.Annotations, cmpopts.EquateEmpty()); len(diff) > 0 {
				klog.V(2).Infof("Secret %s/%s annotations diff: %s", actualTargetCertKeyPairSecret.Namespace, actualTargetCertKeyPairSecret.Name, diff)
			}
		}
