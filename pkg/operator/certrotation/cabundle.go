...
func (c CABundleConfigMap) EnsureConfigMapCABundle(ctx context.Context, signingCertKeyPair *crypto.CA, signingCertKeyPairLocation string) ([]*x509.Certificate, error) {
	...
	if creationRequired {
		...
		if klog.V(2).Enabled() {
			klog.V(2).Infof("Created ca-bundle.crt configmap %s/%s with:\n%s", actualCABundleConfigMap.Namespace, actualCABundleConfigMap.Name, certs.CertificateBundleToString(updatedCerts))
			if diff := cmp.Diff(nil, actualCABundleConfigMap.Annotations, cmpopts.EquateEmpty()); len(diff) > 0 {
		...
		if klog.V(2).Enabled() {
			...
			if diff := cmp.Diff(originalCABundleConfigMap.Annotations, actualCABundleConfigMap.Annotations, cmpopts.EquateEmpty()); len(diff) > 0 {
				klog.V(2).Infof("ConfigMap %s/%s annotations diff: %s", actualCABundleConfigMap.Namespace, actualCABundleConfigMap.Name, diff)
			}
		}
