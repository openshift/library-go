...
func (c *Controller) sync(ctx context.Context, controllerContext factory.SyncContext) error {
	...
	workload, operatorConfigAtHighestGeneration, removeConditions, removedWorkloadName, removedWorkloadNamespace, errs := c.delegate.Sync(ctx, controllerContext)

	return c.updateOperatorStatus(ctx, operatorStatus, workload, operatorConfigAtHighestGeneration, true, removeConditions, removedWorkloadName, removedWorkloadNamespace, errs)
}
...
func (c *Controller) updateOperatorStatus(ctx context.Context,
	...
	operatorConfigAtHighestGeneration, preconditionsReady, removeConditions bool,
	...
	mustApplyStatus := false
	statusCleanupPatch := jsonpatch.New()

	// remove operator status conditions if requested by the delegate; otherwise update them
	// if necessary
	if removeConditions {
		statusCleanupPatch = v1helpers.RemoveConditionsJSONPatch(previousStatus, []string{typeAvailable, typeDegraded, typeProgressing, typeWorkloadDegraded}, statusCleanupPatch)

	} else {
		status = c.updateOperatorStatusConditions(previousStatus, status, workload, preconditionsReady, deploymentAvailableCondition, deploymentDegradedCondition, deploymentProgressingCondition, workloadDegradedCondition, errs)
		mustApplyStatus = true
	}

	// remove operator status generations and version if requested by the delegate; otherwise update them
	// if necessary
	if len(removedWorkloadName) > 0 && len(removedWorkloadNamespace) > 0 {
		statusCleanupPatch = v1helpers.RemoveWorkloadGenerationsJSONPatch(previousStatus, removedWorkloadName, removedWorkloadNamespace, statusCleanupPatch)

		c.setVersion(removedWorkloadName, "")

	} else {
		status = c.updateOperatorStatusGenerationsVersion(status, workload, operatorConfigAtHighestGeneration, preconditionsReady)
		mustApplyStatus = true
	}
	...
	if !statusCleanupPatch.IsEmpty() {
		if patchErr := c.operatorClient.PatchOperatorStatus(ctx, statusCleanupPatch); patchErr != nil {
			return patchErr
		}
	}
