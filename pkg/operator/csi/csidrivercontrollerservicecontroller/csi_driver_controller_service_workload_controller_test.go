...
func TestWorkloadHook(t *testing.T) {
	...
	kubeInformers := v1helpers.NewKubeInformersForNamespaces(coreClient, operandNamespace)
	...
	namespacedInformerFactory := coreinformers.NewSharedInformerFactoryWithOptions(coreClient, 0, coreinformers.WithNamespace(operandNamespace))

	controller := NewCSIDriverControllerServiceWorkloadController(
		controllerName,
		operandNamespace,
		makeFakeManifest(),
...
func getPreconditions() []dc.PreconditionFunc {
	return []dc.PreconditionFunc{
