...
func TestFromEncryptionState(t *testing.T) {
	...
	for _, scenario := range scenarios {
		t.Run(scenario.name, func(t *testing.T) {
			...
			// Ignore KMS Name field since it contains a random suffix
			ignoreKMSName := cmpopts.IgnoreFields(apiserverconfigv1.KMSConfiguration{}, "Name")
			if !cmp.Equal(expectedOutput, actualOutput.Resources, ignoreKMSName) {
				t.Fatal(fmt.Errorf("%s", cmp.Diff(expectedOutput, actualOutput.Resources, ignoreKMSName)))
			}
		})
	}
}
...
func keyToKMSConfiguration(key *corev1.Secret, resource string) *apiserverconfigv1.KMSConfiguration {
	...
	return &apiserverconfigv1.KMSConfiguration{
		APIVersion: "v2",
		Name:       fmt.Sprintf("%d-test%d", keyID, keyID),
