...
func TestKeyController(t *testing.T) {
	...
	scenarios := []struct {
			...
			validateFunc: func(ts *testing.T, actions []clientgotesting.Action, targetNamespace string, targetGRs []schema.GroupResource) {
				...
				for _, action := range actions {
					if action.Matches("create", "secrets") {
						...
						if kmsConfig != `{"endpoint":"unix:///var/run/kmsplugin/kms.sock"}` {
...

		{
			...
			initialObjects: []runtime.Object{
				encryptiontesting.CreateDummyKubeAPIPod("kube-apiserver-1", "kms", "node-1"),
				encryptiontesting.CreateExpiredMigratedEncryptionKeySecretWithKMSConfig("kms", []schema.GroupResource{{Group: "", Resource: "secrets"}}, 5, []byte("kms-checksum-data"), `{"endpoint":"unix:///var/run/kmsplugin/kms.sock"}`),
				encryptiontesting.CreateEncryptionKeySecretWithKMSConfig("kms", nil, 6, []byte("kms-checksum-data"), `{"endpoint":"unix:///var/run/kmsplugin/kms.sock"}`),
			},
			apiServerObjects: []runtime.Object{apiServerWithKMS},
			targetNamespace:  "kms",
		...
		{
			name: "creates a new KMS key when KMS configuration changes",
			targetGRs: []schema.GroupResource{
				{Group: "", Resource: "secrets"},
			},
			initialObjects: []runtime.Object{
				encryptiontesting.CreateDummyKubeAPIPod("kube-apiserver-1", "kms", "node-1"),
				encryptiontesting.CreateEncryptionKeySecretWithKMSConfig("kms", []schema.GroupResource{{Group: "", Resource: "secrets"}}, 5, []byte("old-kms-checksum"), `{"endpoint":"unix:///var/run/kmsplugin/old-kms.sock"}`),
			},
			apiServerObjects: []runtime.Object{apiServerWithKMS},
			targetNamespace:  "kms",
			expectedActions:  []string{"list:pods:kms", "get:secrets:kms", "list:secrets:openshift-config-managed", "create:secrets:openshift-config-managed", "create:events:kms"},
			validateFunc: func(ts *testing.T, actions []clientgotesting.Action, targetNamespace string, targetGRs []schema.GroupResource) {
				...
				for _, action := range actions {
					if action.Matches("create", "secrets") {
						...
						// Verify mode is still KMS
						if actualSecret.Annotations["encryption.apiserver.operator.openshift.io/mode"] != "KMS" {
							ts.Errorf("expected mode to be KMS, got %s", actualSecret.Annotations["encryption.apiserver.operator.openshift.io/mode"])
						}

						// Verify new KMS config annotation
						kmsConfig := actualSecret.Annotations["encryption.apiserver.operator.openshift.io/kms-config"]
						if kmsConfig != `{"endpoint":"unix:///var/run/kmsplugin/kms.sock"}` {
							ts.Errorf("unexpected kms-config: %s, expected default config", kmsConfig)
						}

						// Verify internal reason is kms-config-changed
						if actualSecret.Annotations["encryption.apiserver.operator.openshift.io/internal-reason"] != "secrets-kms-config-changed" {
							ts.Errorf("unexpected internal reason: %s", actualSecret.Annotations["encryption.apiserver.operator.openshift.io/internal-reason"])
						}
...

		{
			...
			initialObjects: []runtime.Object{
				encryptiontesting.CreateDummyKubeAPIPod("kube-apiserver-1", "kms", "node-1"),
				encryptiontesting.CreateEncryptionKeySecretWithKMSConfig("kms", nil, 3, []byte("kms-checksum-data"), `{"endpoint":"unix:///var/run/kmsplugin/kms.sock"}`),
			},
			apiServerObjects: []runtime.Object{apiServerWithKMS},
			targetNamespace:  "kms",
...

		{
			...
			initialObjects: []runtime.Object{
				encryptiontesting.CreateDummyKubeAPIPod("kube-apiserver-1", "kms", "node-1"),
				encryptiontesting.CreateEncryptionKeySecretWithKMSConfig("kms", []schema.GroupResource{{Group: "", Resource: "secrets"}}, 7, []byte("kms-checksum-data"), `{"endpoint":"unix:///var/run/kmsplugin/kms.sock"}`),
			},
			apiServerObjects: []runtime.Object{apiServerWithAESCBC},
			targetNamespace:  "kms",
			expectedActions:  []string{"list:pods:kms", "get:secrets:kms", "list:secrets:openshift-config-managed", "create:secrets:openshift-config-managed", "create:events:kms"},
		...
		{
			name: "degraded when a KMS secret with invalid checksum exists",
			targetGRs: []schema.GroupResource{
				{Group: "", Resource: "secrets"},
			},
			initialObjects: []runtime.Object{
				encryptiontesting.CreateDummyKubeAPIPod("kube-apiserver-1", "kms", "node-1"),
				encryptiontesting.CreateEncryptionKeySecretWithKMSConfig("kms", nil, 1, []byte(""), `{"endpoint":"unix:///var/run/kmsplugin/kms.sock"}`),
			},
			apiServerObjects: []runtime.Object{apiServerWithKMS},
			targetNamespace:  "kms",
			expectedActions:  []string{"list:pods:kms", "get:secrets:kms", "list:secrets:openshift-config-managed", "create:secrets:openshift-config-managed", "get:secrets:openshift-config-managed"},
...
func TestGetCurrentModeAndExternalReason(t *testing.T) {
	scenarios := []struct {
...

		{
			...
			expectedKMSConfig: []byte(`{"endpoint":"unix:///var/run/kmsplugin/kms.sock"}`),
		},
	}
	...
	for _, scenario := range scenarios {
		t.Run(scenario.name, func(t *testing.T) {
			...
			_, externalReason, kmsConfig, err := target.getCurrentModeAndExternalReason(context.TODO())
			...
			if string(kmsConfig) != string(scenario.expectedKMSConfig) {
				t.Errorf("unexpected kmsConfig: %q, expected: %q", string(kmsConfig), string(scenario.expectedKMSConfig))
			}
		})
	}
}
