			...
			validateFunc: func(ts *testing.T, actions []clientgotesting.Action, targetNamespace string, targetGRs []schema.GroupResource) {
				...
				for _, action := range actions {
					if action.Matches("create", "secrets") {
						...
						expectedProviderConfig, err := json.Marshal(kms.GetKMSProviderConfig(apiServerWithKMS))
						if err != nil {
...
func TestGetCurrentModeAndExternalReason(t *testing.T) {
	...
	scenarios := []struct {
...

		{
			...
			expectedKMSProviderConfig: kms.GetKMSProviderConfig(kmsAPIServer),
		},
	}
	...
	for _, scenario := range scenarios {
		t.Run(scenario.name, func(t *testing.T) {
			...
			currentMode, externalReason, kmsProviderConfig, err := target.getCurrentModeAndExternalReason(context.TODO())
			...
			if !equality.Semantic.DeepEqual(kmsProviderConfig, scenario.expectedKMSProviderConfig) {
				t.Errorf("unexpected KMS provider config:\n%s", diff.Diff(scenario.expectedKMSProviderConfig, kmsProviderConfig))
			}
		})
	}
}
