...
func (c *stateController) generateAndApplyCurrentEncryptionConfigSecret(ctx context.Context, queue workqueue.RateLimitingInterface, recorder events.Recorder, encryptedGRs []schema.GroupResource) error {
	...
	kmsProviderConfigs, err := collectKMSProviderConfigs(desiredEncryptionState)
	if err != nil {
		return err
	}
...
// collectKMSProviderConfigs iterates over active encryption providers and returns
// KMS provider configs keyed by keyID for those in KMS encryption mode.
func collectKMSProviderConfigs(desiredState map[schema.GroupResource]state.GroupResourceState) (map[string]*state.KMSProviderConfig, error) {
	...
	for _, grState := range desiredState {
		for _, ks := range grState.ReadKeys {
			...
			if ks.KMSProviderConfig == nil {
				return nil, fmt.Errorf("KMSProviderConfig should not be empty for key %s in KMS mode", ks.Key.Name)
			}
