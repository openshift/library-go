...
func reconcileRemoteKeyRotation(
	...
	if !secrets.IsBootstrapped(rk) {
		...
		err := secrets.PatchRemoteKeyState(ctx, secretClient.Secrets("openshift-config-managed"), secretName, func(rk *state.RemoteKeyState) (bool, error) {
			if secrets.IsBootstrapped(*rk) {
				return false, nil
			}
			if len(rk.TargetRemoteKeyID) == 0 {
...
func readRemoteKeyAnnotationsFromSecret(ctx context.Context, secretClient corev1client.SecretsGetter, secretName string) (state.RemoteKeyState, error) {
...
func shouldPromoteConvergedKeyID(rk state.RemoteKeyState, candidateRemoteKeyID string, now time.Time) (bool, error) {
...
func clearRemoteKeyConvergence(ctx context.Context, secretClient corev1client.SecretsGetter, secretName string, rk state.RemoteKeyState) error {
