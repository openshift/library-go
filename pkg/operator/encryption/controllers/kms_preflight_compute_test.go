...
func TestKMSPreflightComputeEncryptionConfiguration(t *testing.T) {
	...
	t.Run("first key, no existing secrets, produces key ID 1", func(t *testing.T) {
		...
		secret, err := computer.ComputeEncryptionConfiguration(context.TODO(), nil, 0)
		if err != nil {
	...
	t.Run("EncryptedGRs are read at compute time", func(t *testing.T) {
		...
		first, err := computer.ComputeEncryptionConfiguration(context.TODO(), nil, 0)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		...
		second, err := computer.ComputeEncryptionConfiguration(context.TODO(), nil, 0)
		if err != nil {
