...
func (c *keyController) generateKeySecret(ctx context.Context, keyID uint64, currentMode state.Mode, apiServerEncryption configv1.APIServerEncryption, desiredProviderCfg kmsProviderConfig, internalReason, externalReason string) (*corev1.Secret, bool, error) {
	result, err := buildEncryptionKeyState(
		...
		internalReason,
		externalReason,
		"", // use the real operand endpoint format
	)
	if err != nil {
	...
	if currentMode == state.KMS {
		// Reuse the Secret/ConfigMap already fetched by buildEncryptionKeyState
		// so hashing does not require a second API round-trip.
		resources := &prefetchedKMSConfigHasherResourceProvider{secret: result.refSecret, configMap: result.refCM}
	...
	secret, err := secrets.FromKeyState(c.instanceName, result.keyState)
	if err != nil {
		return nil, false, err
	}
