...
type validationFunc func(context.Context, *authenticationv1alpha1.AuthenticationConfiguration) error
...
// ExternalClaimsSourcingOptions configures external claims sourcing. A nil
// value disables external claims sourcing.
type ExternalClaimsSourcingOptions struct {
	// ClientSecretResolver resolves client Secrets for sources that use client
	// credential authentication. It may be nil when no such source is configured.
	ClientSecretResolver ClientSecretResolver
}

// GeneratorOptions configures an AuthenticationConfigurationGenerator.
type GeneratorOptions struct {
	CertificateAuthorityResolver CertificateAuthorityResolver
...
// lookups itself.
func NewAuthenticationConfigurationGenerator(options GeneratorOptions) *AuthenticationConfigurationGenerator {
	generator := &AuthenticationConfigurationGenerator{
		caResolver:                    options.CertificateAuthorityResolver,
		externalClaimsSourcingEnabled: options.ExternalClaimsSourcing != nil,
		validationFn:                  validateOAuthApiserverAuthenticationConfiguration,
	}
	if options.ExternalClaimsSourcing != nil {
		generator.clientSecretResolver = options.ExternalClaimsSourcing.ClientSecretResolver
	}
...
//
// The context controls the reachability checks made against configured OIDC issuers, allowing callers
// to cancel generation when their reconciliation is canceled. It must not be nil.
func (acg *AuthenticationConfigurationGenerator) GenerateAuthenticationConfiguration(ctx context.Context, authSpec *configv1.AuthenticationSpec) (*authenticationv1alpha1.AuthenticationConfiguration, error) {
	if authSpec == nil {
	...
	if acg.validationFn != nil {
		if err := acg.validationFn(ctx, authConfig); err != nil {
			return nil, err
		}
	}
...
func validateOAuthApiserverAuthenticationConfiguration(ctx context.Context, auth *authenticationv1alpha1.AuthenticationConfiguration) error {
	if auth == nil {
		return nil
	}
	...
	for _, jwt := range auth.JWT {
		...
		if err := validateCACert(ctx, url, caCertPool); err != nil {
...
func validateCACert(ctx context.Context, hostURL string, caCertPool *x509.CertPool) error {
	client := &http.Client{
	...
	req, err := http.NewRequestWithContext(retryCtx, http.MethodGet, hostURL, nil)
	if err != nil {
