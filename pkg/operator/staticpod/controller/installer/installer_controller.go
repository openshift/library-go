	...
	for l := 0; l < len(operatorStatus.NodeStatuses); l++ {
		...
		if currNodeState.TargetRevision > currNodeState.CurrentRevision {
			...
			if !equality.Semantic.DeepEqual(newCurrNodeState, currNodeState) {
				...
				nodeStatusApplyConfigurations := prepareNodeStatusApplyConfigurationForUpdatedNode(originalOperatorStatus.NodeStatuses, *newCurrNodeState)
				...
				operatorConditionApplyConfigurations = append(operatorConditionApplyConfigurations, prepareInstallerDegradedConditionApplyConfigurationFor(nil))
...
func prepareNodeStatusApplyConfigurationForUpdatedNode(nodeStatuses []operatorv1.NodeStatus, updatedNodeStatus operatorv1.NodeStatus) []*applyoperatorv1.NodeStatusApplyConfiguration {
	...
	for _, nodeStatus := range nodeStatuses {
		if nodeStatus.NodeName == updatedNodeStatus.NodeName {
...
func (c InstallerController) Sync(ctx context.Context, syncCtx factory.SyncContext) error {
	...
	nodeStatusApplyConfigurations := nodeStatusesToNodeStatusApplyConfigurations(originalOperatorStatus.NodeStatuses)
	operatorConditionApplyConfigurations := prepareAvailableProgressingNodeInstallerFailingConditionsApplyConfigurations(nodeStatusApplyConfigurations, originalOperatorStatus.LatestAvailableRevision)
