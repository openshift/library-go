...
func TestObserveTLSSecurityProfileWithGroupPaths(t *testing.T) {
	// The observer emits the profile's groups verbatim; FIPS-mode omission is the
	// consuming component's job, so these expectations are identical in both FIPS
	// and non-FIPS runtimes.
	...
	testCases := []struct {
		...
		{
			// A TLS 1.3-only ML-KEM hybrid is honored verbatim: the observer neither
			// drops it nor injects a classical fallback.
			...
			expectedGroups: []string{"X25519MLKEM768"},
		},
	}
