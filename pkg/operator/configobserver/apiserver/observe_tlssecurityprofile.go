...
func innerTLSSecurityProfileObservations(genericListers configobserver.Listers, recorder events.Recorder, existingConfig map[string]interface{}, minTLSVersionPath, cipherSuitesPath, groupsPath []string) (ret map[string]interface{}, _ []error) {
	...
	if len(groupsPath) > 0 {
		...
		// Store the groups verbatim, including an empty list. Per the TLS curves
		// enhancement the administrator's list is honored as-is; FIPS-mode omission
		// is the consuming component's responsibility, and an empty result lets TLS
		// fail rather than negotiating unrequested curves.
...
// CLI arguments.
//
// The groups are returned verbatim: no groups are dropped and none are injected.
// FIPS-mode omission of non-approved groups is the consuming component's
// responsibility (crypto.IsFIPSEnabled + crypto.FIPSApprovedTLSGroups), matching
// the api-tls-curves-config enhancement which places that filtering on the
// component, not on this observer. If the resulting groups cannot negotiate with a
// peer the handshake is expected to fail rather than fall back to unrequested
// curves.
