...
// ObjectQueueKeysFunc is useds to make a string slice of work queue key out of the runtime object that is passed to it.
// Each key in the slice can extract the "namespace/name" if you need to or just return "key" if you building controller that only use string
// triggers.
...
func (f *Factory) WithInformersQueueKeyFunc(queueKeyFn ObjectQueueKeyFunc, informers ...Informer) *Factory {
	f.informerQueueKeys = append(f.informerQueueKeys, informersWithQueueKey{
		...
		queueKeysFn: func(obj runtime.Object) []string {
			return []string{queueKeyFn(obj)}
		},
	})
	return f
}

// WithMultipleInformersQueueKeysFunc is used to register event handlers and get the caches synchronized functions.
// Pass informers you want to use to react to changes on resources. If informer event is observed, then the Sync() function
// is called.
// Pass the queueKeysFn you want to use to transform the informer runtime.Object into a string slice of keys used by work queue.
func (f *Factory) WithMultipleInformersQueueKeysFunc(queueKeysFn ObjectQueueKeysFunc, informers ...Informer) *Factory {
	f.informerQueueKeys = append(f.informerQueueKeys, informersWithQueueKey{
		informers:   informers,
		queueKeysFn: queueKeysFn,
	})
	return f
}
...
func (f *Factory) ToController(name string, eventRecorder events.Recorder) Controller {
	...
	for i := range f.informerQueueKeys {
		for d := range f.informerQueueKeys[i].informers {
			...
			queueKeysFn := f.informerQueueKeys[i].queueKeysFn
			informer.AddEventHandler(c.syncContext.(syncContext).eventHandler(queueKeysFn, f.informerQueueKeys[i].filter))
